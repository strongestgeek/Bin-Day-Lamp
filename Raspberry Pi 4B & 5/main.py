...
# I think this definition needs to be rewritten to show the colours on the right sides of the LED HAT.
...
with open('colours.txt', 'r') as file:
    for line in file:
        ...
        if len(values) == 4:  # If there's only one set of RGB values
            color = Color(int(values[1]), int(values[2]), int(values[3]))  # Single color
            colors[date] = (color, color)  # Use the same color for both halves
        elif len(values) == 7:  # If there are two sets of RGB values
...
if today in colors:
    ...
    print(f"LED colours changed for {today} with rotated two colors")
else:
